...
def cleanDuplicates():
    ...
    statementQuery = """DELETE FROM statementsData_201710
        WHERE revId IN
            (SELECT revId FROM (SELECT revId,
    ...
    referenceQuery = """DELETE FROM referenceData_201710
            WHERE revId IN
                (SELECT revId FROM (SELECT revId,
                     ROW_NUMBER() OVER( PARTITION BY referenceId, statementId, refProperty, refValue
                    ORDER BY  revId ) AS row_num
    ...
    WHERE revId IN
        (SELECT revId FROM (SELECT revId,
             ROW_NUMBER() OVER( PARTITION BY statementId, qualifierId, qualProperty, qualValue
            ORDER BY  revId ) AS row_num
